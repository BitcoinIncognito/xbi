...
class BudgetProposalTest(xbiTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test with invalid xbi address")
        assert_raises_rpc_error(-5, "Invalid xbi address", self.nodes[0].preparebudget,
                                name, scheme + url, numcycles, nextsuperblock, "DBREvBMNQguwuC4YMoCG5FoH1sA2YntvZm", cycleamount)

        self.log.info("Test with too low amount")
        invalid_amt = 9.99999999
        assert_raises_rpc_error(-8, "Invalid amount - Payment of %.8f is less than minimum 10 XBI allowed" % invalid_amt, self.nodes[0].preparebudget,
