...
#if defined(NDEBUG)
#error "xbi cannot be compiled without assertions."
#endif
...
CAmount GetBlockValue(int nHeight)
{
   ...
   int64_t nBlockValue = 1 * COIN;

    if (nHeight == 92473) {
        return 1206657 * COIN;
    }

    // DatBer coinBurn 2018
    if (nHeight == 294851) {
        return 55000 * COIN;
    }

    if (nHeight == 0) {
        nBlockValue = 2100000 * COIN;
...
{
    util::ThreadRename("xbi-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!fInitialBlockDownload) {
        if (!IsBlockPayeeValid(block, pindex->pprev, nHeight)) {
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (!Params().IsRegTestNet() && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 222)) {
    ...
    if (block.nBits != nBitsRequired) {
        // xbi Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nxbiBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nxbiBadBlockBits)) {
            // accept xbi block minted with incorrect proof of work threshold
            return true;
        }
        ...
        for (const CTxIn& in: tx->vin) {
            ...
            if (isPrivZerocoinSpend || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZXBIModule::ParseZerocoinPublicSpend(in, *tx, state, publicSpend)){
                        return false;
                    }
                    spend = publicSpend;
                } else {
