...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select XBI Outputs to Spend" :
            "Select Shielded XBI to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::XBI;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show xbi address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
