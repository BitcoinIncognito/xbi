...
void OptionsModel::setNetworkDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("fUseUPnP") || reset)
        settings.setValue("fUseUPnP", DEFAULT_UMNP);
...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::XBI);
    ...
    if (settings.contains("nAnonymizexbiAmount") || reset)
        gArgs.SoftSetArg("-anonymizexbiamount", settings.value("nAnonymizexbiAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in xbi.cpp during shotdown to also remove the window positions
