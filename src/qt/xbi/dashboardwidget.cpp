...
#include "qt/xbi/forms/ui_dashboardwidget.h"
#include "qt/xbi/sendconfirmdialog.h"
#include "qt/xbi/txrow.h"
...
DashboardWidget::DashboardWidget(xbiGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &xbiGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("XBI")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair XBI, MN Reward
...
void DashboardWidget::onChartRefreshed()
{
    ...
    if (chartData->totalPiv > 0 || chartData->totalMN > 0) {
        ...
        setCssProperty(ui->labelAmountMN, "text-stake-pn");
    } else {
        setCssProperty(ui->labelAmountPiv, "text-stake-piv-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-pn-disable");
    }
    ...
    if (fShowCharts) {
        if (!stakesFilter) {
            ...
            stakesFilter->setTypeFilter(TransactionFilterProxy::TYPE(TransactionRecord::StakeMint) |
                                        ...
                                        TransactionFilterProxy::TYPE(TransactionRecord::StakeZXBI) |
