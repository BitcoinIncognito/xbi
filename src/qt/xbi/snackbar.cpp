...
#include "qt/xbi/forms/ui_snackbar.h"
#include "qt/xbi/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(xbiGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &xbiGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
