...
#include "qt/xbi/send.h"
#include "qt/xbi/forms/ui_send.h"
#include "qt/xbi/addnewcontactdialog.h"
#include "qt/xbi/qtutils.h"
#include "qt/xbi/sendchangeaddressdialog.h"
#include "qt/xbi/optionbutton.h"
#include "qt/xbi/sendconfirmdialog.h"
#include "qt/xbi/guitransactionsutils.h"
#include "qt/xbi/loadingdialog.h"
...
SendWidget::SendWidget(xbiGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a xbi URI"));
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onXBISelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onXBISelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent XBIs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield XBIs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent XBIs to shield."));
    }
}
...
void SendWidget::onXBISelected(bool _isTransparent)
{
