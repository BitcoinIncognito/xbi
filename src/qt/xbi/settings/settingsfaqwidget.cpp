// Copyright (c) 2019 The xbi developers
...
#include "qt/xbi/settings/settingsfaqwidget.h"
#include "qt/xbi/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(xbiGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableXBI,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableXBI,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("xbi is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. xbi incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("xbi utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of xbi to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableXBIContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received xbi requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your xbi wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableXBI->setText(unspendableXBIContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of xbi with a minimum of 480 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("xbi-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::XBI)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in xbi Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the xbi network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableXBI, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableXBI);});
    ...
    if (parent)
        connect(parent, &xbiGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
