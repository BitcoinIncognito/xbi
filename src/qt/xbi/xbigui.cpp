...
#include "qt/xbi/qtutils.h"
#include "qt/xbi/defaultdialog.h"
...
const QString xbiGUI::DEFAULT_WALLET = "~Default";

xbiGUI::xbiGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void xbiGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &xbiGUI::toggleHidden);
...
void xbiGUI::connectActions()
{
    ...
    connect(topBar, &TopBar::showHide, this, &xbiGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &xbiGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &xbiGUI::showHide);
    connect(sendWidget, &SendWidget::showHide, this, &xbiGUI::showHide);
    connect(receiveWidget, &ReceiveWidget::showHide, this, &xbiGUI::showHide);
    connect(addressesWidget, &AddressesWidget::showHide, this, &xbiGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::showHide, this, &xbiGUI::showHide);
    connect(masterNodesWidget, &MasterNodesWidget::execDialog, this, &xbiGUI::execDialog);
    connect(coldStakingWidget, &ColdStakingWidget::showHide, this, &xbiGUI::showHide);
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &xbiGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &xbiGUI::execDialog);
}


void xbiGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
#ifndef Q_OS_MAC
...
void xbiGUI::handleRestart(QStringList args)
{
...
void xbiGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &xbiGUI::message);
...
void xbiGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &xbiGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &xbiGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void xbiGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void xbiGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
...
void xbiGUI::changeEvent(QEvent* e)
{
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &xbiGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &xbiGUI::show);
                e->ignore();
            }
        }
    }
...
void xbiGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void xbiGUI::messageInfo(const QString& text)
{
...
void xbiGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret)
{
...
bool xbiGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
...
void xbiGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if (!clientModel)
        return;
...
void xbiGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void xbiGUI::detectShutdown()
{
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
...
void xbiGUI::goToSend()
{
    showTop(sendWidget);
}

void xbiGUI::goToAddresses()
{
    showTop(addressesWidget);
}

void xbiGUI::goToMasterNodes()
{
    showTop(masterNodesWidget);
}

void xbiGUI::goToColdStaking()
{
    showTop(coldStakingWidget);
}

void xbiGUI::goToSettings(){
    showTop(settingsWidget);
}

void xbiGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void xbiGUI::goToReceive()
{
    showTop(receiveWidget);
}

void xbiGUI::openNetworkMonitor()
{
    settingsWidget->openNetworkMonitor();
}

void xbiGUI::showTop(QWidget* view)
{
...
void xbiGUI::changeTheme(bool isLightTheme)
{
...
void xbiGUI::resizeEvent(QResizeEvent* event)
{
    // Parent..
...
bool xbiGUI::execDialog(QDialog *dialog, int xDiv, int yDiv)
{
    return openDialogWithOpaqueBackgroundY(dialog, this);
}

void xbiGUI::showHide(bool show)
{
...
int xbiGUI::getNavWidth()
{
    return this->navMenu->width();
}

void xbiGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
bool xbiGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &xbiGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &xbiGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &xbiGUI::message);
    connect(topBar, &TopBar::message, this, &xbiGUI::message);
    connect(sendWidget, &SendWidget::message,this, &xbiGUI::message);
    connect(receiveWidget, &ReceiveWidget::message,this, &xbiGUI::message);
    connect(addressesWidget, &AddressesWidget::message,this, &xbiGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &xbiGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &xbiGUI::incomingTransaction);

    return true;
}

bool xbiGUI::setCurrentWallet(const QString& name)
{
    // Single wallet supported.
    return true;
}

void xbiGUI::removeAllWallets()
{
    // Single wallet supported.
}

void xbiGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
...
static bool ThreadSafeMessageBox(xbiGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void xbiGUI::subscribeToCoreSignals()
{
...
void xbiGUI::unsubscribeFromCoreSignals()
{
