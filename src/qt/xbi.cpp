...
#include "qt/xbi/splash.h"
#include "qt/xbi/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("xbi-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in xbi.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in xbi.qrc)
...
/** Class encapsulating xbi Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (xbiGUI)
...
{
    window = new xbiGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &xbiGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &xbiGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(xbiGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(xbiGUI::DEFAULT_WALLET);
        }
#endif
        ...
        // xbi: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &xbiGUI::handlePaymentRequest);
        connect(window, &xbiGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. xbi can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data and blocks directory and parse xbi.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", xbi_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // xbi: links repeatedly have their payment requests routed to this process:
