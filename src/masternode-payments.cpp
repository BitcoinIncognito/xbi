...
bool IsBlockPayeeValid(const CBlock& block, const CBlockIndex* pindexPrev, int nHeight)
{
    ...
    if (masternodePayments.IsTransactionValid(txNew, pindexPrev, nHeight))
        return true;
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const CBlockIndex* pindexPrev, bool fProofOfStake) const
{
    ...
    // Starting from xbi v6.0 masternode and budgets are paid in the coinbase tx
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, int nHeight)
{
    LOCK(cs_vecPayments);
...
bool CMasternodePayments::IsTransactionValid(const CTransaction& txNew, const CBlockIndex* pindexPrev, int nHeight)
{
    ...
    if (mapMasternodeBlocks.count(nBlockHeight)) {
        return mapMasternodeBlocks[nBlockHeight].IsTransactionValid(txNew, nHeight);
    }
