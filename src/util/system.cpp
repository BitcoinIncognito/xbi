...
const char * const xbi_CONF_FILENAME = "xbi.conf";
const char * const xbi_PID_FILENAME = "xbi.pid";
const char * const xbi_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "xbid -foo=bar
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\xbi
// Windows >= Vista: C:\Users\Username\AppData\Roaming\xbi
// Mac: ~/Library/Application Support/xbi
// Unix: ~/.xbi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XBI";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\xbiParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\xbiParams
    // Mac: ~/Library/Application Support/xbiParams
    // Unix: ~/.xbi-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "xbiParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "xbiParams";
#else
    // Unix
    return pathRet / ".xbi-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/xbi/sapling-spend.params";
        sapling_output = "/usr/share/xbi/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/xbi/sapling-spend.params";
            sapling_output = "/usr/local/share/xbi/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", xbi_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        FILE* configFile = fsbridge::fopen(pathConfigFile, "a");
         if (configFile != NULL) {
            std::string strHeader = "# xbi config file\n";
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", xbi_PID_FILENAME));
