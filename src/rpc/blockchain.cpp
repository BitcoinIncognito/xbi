...
UniValue getsupplyinfo(const JSONRPCRequest& request)
{
    ...
    const CAmount tSupply = MoneySupply.Get() - (DEFAULT_BLOCK_MAX_SIZE * 6 * COIN);
...
UniValue gettxoutsetinfo(const JSONRPCRequest& request)
{
    ...
    CCoinsStats stats;
    FlushStateToDisk();
    if (GetUTXOStats(pcoinsTip.get(), stats) && sporkManager.IsSporkActive(SPORK_20_SAPLING_MAINTENANCE)) {
        ret.pushKV("height", (int64_t)stats.nHeight);
        ret.pushKV("bestblock", stats.hashBlock.GetHex());
        ret.pushKV("transactions", (int64_t)stats.nTransactions);
        ret.pushKV("txouts", (int64_t)stats.nTransactionOutputs);
        ret.pushKV("hash_serialized_2", stats.hashSerialized.GetHex());
        ret.pushKV("total_amount", ValueFromAmount(stats.nTotalAmount-(DEFAULT_BLOCK_MAX_SIZE * 6 * COIN)));
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"value\" : x.xxx,           (numeric) The transaction value in XBI\n"
            "  \"scriptPubKey\" : {         (json object)\n"
            "     \"asm\" : \"code\",       (string) \n"
            "     \"hex\" : \"hex\",        (string) \n"
            "     \"reqSigs\" : n,          (numeric) Number of required signatures\n"
            "     \"type\" : \"pubkeyhash\", (string) The type, eg pubkeyhash\n"
            "     \"addresses\" : [          (array of string) array of xbi addresses\n"
