...
// Copyright (c) 2015-2021 The xbi developers
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", xbi_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", xbi_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-upnp", strprintf("Use UPnP to map the listening port (default: %u)", DEFAULT_UMNP));
#ifdef USE_NATPMP
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all xbi specific functionality (Masternodes, Budgeting) (0-1, default: %u)", 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify masternode configuration file (default: %s)", xbi_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf("Set external address:port to get to this masternode (example: %s)", "128.127.106.235:7339"));
...
/** Sanity checks
 *  Ensure that xbi is running in a usable environment with all
...
{
    // Make sure only a single xbi process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", xbi_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if xbi is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if xbi is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //xbi specific: zerocoin and spork DB's
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    StartMapPort(gArgs.GetBoolArg("-upnp", DEFAULT_UMNP), gArgs.GetBoolArg("-natpmp", DEFAULT_NATPMP));
