...
/* FIXME: The macros using external routines like __MMN(count_leading_zeros)
...
long __MMN(count_leading_zeros) (UDItype) __attribute__ ((const));
#else
long __MMN(count_leading_zeros) (UDItype);
#endif
#define count_leading_zeros(count, x) \
  ((count) = __MMN(count_leading_zeros) (x))
#endif /* clz using mpn */
#endif /* __alpha */
...
#define udiv_qrnnd(q, r, n1, n0, d) \
  do { UWtype __r;							\
    (q) = __MMN(udiv_qrnnd) (&__r, (n1), (n0), (d));			\
    (r) = __r;								\
  } while (0)
extern UWtype __MMN(udiv_qrnnd) (UWtype *, UWtype, UWtype, UWtype);
#endif /* LONGLONG_STANDALONE */
#else /* ARMv4 or newer */
#define umul_ppmm(xh, xl, a, b) \
...
#define mpn_umul_ppmm  __MMN(umul_ppmm)
...
#define mpn_umul_ppmm_r  __MMN(umul_ppmm_r)
...
#define mpn_udiv_qrnnd  __MMN(udiv_qrnnd)
...
#define mpn_udiv_qrnnd_r  __MMN(udiv_qrnnd_r)
...
#define udiv_qrnnd(q, r, nh, nl, d) \
  do {									\
    UWtype __r;								\
    (q) = __MMN(udiv_w_sdiv) (&__r, nh, nl, d);				\
    (r) = __r;								\
  } while (0)
__GMP_DECLSPEC UWtype __MMN(udiv_w_sdiv) (UWtype *, UWtype, UWtype, UWtype);
#endif
