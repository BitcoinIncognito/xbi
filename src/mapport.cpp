...
static_assert(MINIUMNPC_API_VERSION >= 10, "miniUPnPc API version >= 10 assumed");
#endif // USE_UMNP
...
#if defined(USE_NATPMP) || defined(USE_UMNP)
...
static bool ProcessUpnp()
{
    ...
    struct UMNPDev* devlist = nullptr;
    char lanaddr[64];

    int error = 0;
#if MINIUMNPC_API_VERSION < 14
    devlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, &error);
#else
    devlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, 2, &error);
#endif

    struct UMNPUrls urls{};
    struct IGDdatas data{};
    int r;

    r = UMNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr));
    if (r == 1) {
        if (fDiscover) {
            char externalIPAddress[40];
            r = UMNP_GetExternalIPAddress(urls.controlURL, data.first.servicetype, externalIPAddress);
            if (r != UMNPCOMMAND_SUCCESS) {
        ...
        do {
            r = UMNP_AddPortMapping(urls.controlURL, data.first.servicetype, port.c_str(), port.c_str(), lanaddr, strDesc.c_str(), "TCP", nullptr, "0");
        ...
        r = UMNP_DeletePortMapping(urls.controlURL, data.first.servicetype, port.c_str(), "TCP", nullptr);
        LogPrintf("UMNP_DeletePortMapping() returned: %d\n", r);
        freeUMNPDevlist(devlist); devlist = nullptr;
        FreeUMNPUrls(&urls);
    } else {
        LogPrintf("No valid UPnP IGDs found\n");
        freeUMNPDevlist(devlist);
        devlist = nullptr;
        if (r != 0)
            FreeUMNPUrls(&urls);
    }
...
static void ThreadMapPort()
{
    ...
    do {
        ...
        if (g_mapport_enabled_protos & MapPortProtoFlag::UMNP) {
            g_mapport_current_proto = MapPortProtoFlag::UMNP;
            ok = ProcessUpnp();
            if (ok) continue;
        }
#endif // USE_UMNP
...
{
    MapPortProtoSetEnabled(MapPortProtoFlag::UMNP, use_upnp);
...
#else  // #if defined(USE_NATPMP) || defined(USE_UMNP)
void StartMapPort(bool use_upnp, bool use_natpmp)
{
...
#endif // #if defined(USE_NATPMP) || defined(USE_UMNP)
