...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("xbiMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("xbiMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("xbiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("xbi-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running xbiMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("xbiMiner exception");
    } catch (...) {
        LogPrintf("xbiMiner exception");
    }

    LogPrintf("xbiMiner exiting\n");
}
