...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " xbi-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  xbi-tx [options] <hex-tx> [commands]  Update hex-encoded xbi transaction\n" +
                               "or:     xbi-tx [options] -create [commands]   Create hex-encoded xbi transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded xbi transaction
