...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/xbiParams"
    else
        PARAMS_DIR="$HOME/.xbi-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
xbi - install-params.sh

This script will install the xbi zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common xbi zkSNARK parameters. Note that it is
