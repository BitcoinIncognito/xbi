...
*Setup instructions for a gitian build of xbi Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the xbi
...
to the xbi GitHub release page.
...
- [Building xbi Core](#building-xbi-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/xbi-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/xbi-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/xbi-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/xbi-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/xbi-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/xbi-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/xbi/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@xbi.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/xbi-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/xbi-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/xbi-osx-*-build.assert
```
